...
<%@ page language="java" pageEncoding="UTF-8"%> 
 ...
 <form id="review_form" action="/admin/review_update_action" method="POST" enctype="multipart/form-data" > 
 ...
 <script> 	
 	...
 	$(document).ready(function() {
 		...
 		$("#btnDelete").click(function() {
 			if(confirm("정말 삭제하시겠습니까?")) {
 	 			var rtnReview = Ajax.SyncPostService("/admin/remove_reivew", {
 	 				seq : seq			
 	 			});
 	 			
 	 			if(rtnReview.Value) {
 	 				document.location = '/admin/review';
 	 			}
 	 			else {
 	 				alert("리뷰 삭제 실패");
 	 			}
 	...
 	function BindImageData(seq) {
 		try
		{
			var rtn = Ajax.SyncPostService("/admin/review_image_list", {
				seq : seq
			});
			Log(rtn);
			$("#div_image").html("");
 	...
 	function remove_image(RV_PHOTO_NUM) {
 		if(confirm("정말 삭제하시겠습니까?")) {
 			var rtnReview = Ajax.SyncPostService("/admin/remove_reivew_image", {
 	...
 	function BindData(seq) { 	
 		var rtnReview = Ajax.SyncPostService("/admin/review_data_detail", {
			seq : seq			
		});
 		Log(rtnReview)
