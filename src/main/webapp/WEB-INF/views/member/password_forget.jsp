...
<form class="form-signin" method="post" name="form" >
  ...
  <button class="btn btn-lg btn-primary btn-block" type="button" id="btnResetPassword">비밀번호 초기화</button>
...
<script>
	...
	function ResetPassword() {
		Ajax.AsyncPostService("/member/reset_password", {
			number : $("#checkNum").val()
			,email : $("#email").val()
			}	
			,function(rtn) {
				...
				if(rtn.Value) {
					if(rtn.Data == 0) {
						alert("초기화에 성공했습니다. 초기화된 비밀번호는 메일로 발송됩니다.");	
